...
The `home-routes` handler in the `test-error.routes.home` namespace
...
The `home-page` function will in turn call the `test-error.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/test_error/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `test-error.handler` namespace:
...
Request middleware functions are located under the `test-error.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `test-error.dev-middleware` namespace found in
